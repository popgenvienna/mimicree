...
class SinglePop:
	"""
	Represetns the allele counts of a sinlge population. Counts are available for the 
	nucleotides A, T, C, G for deletions and for N characters (unknown)
	"""
	...
	def isvalidcoverage(self,mincoverage,maxcoverage):
		if(self.cov < mincoverage or self.cov> maxcoverage):
			return 0
		return 1
	
	def isindel(self, mincount):
		if(self.deletion >= mincount):
			return 1
		return 0
	...
	def cov(self):
		"""
		Only A,T,C,G will be counted.
		Deletions and N characters will be ignored
	...
	def totcov(self):
		"""
		The total coverage including deletions and N characters.
...
class InterPopulationAlleleCounts:
	...
	def subpopulation(self, subpopnumbers):
		"""
		Create a subset out of the selected populations
		"""
		novelpop=[self.populations[i] for i in subpopnumbers]
		toret= InterPopulationAlleleCounts(self.chr,self.pos,self.refc,novelpop)
		return toret
...
class SyncReader:
	"""
	Iterator for reading a .sync file line-by-line.
	...
	def next(self):
		...
		return InterPopulationAlleleCounts(chr,pos,refc,population)
